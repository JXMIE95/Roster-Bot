...
/** Get configured King & Buff roles from DB and fetch them from Discord. */
...
/** Check if the bot can manage a given role. */
...
async function refreshDayEmbed(client, guildId, date) {
  ...
  const slots = Array.from({ length: 24 }, (_, h) => ({
    ...
    locked: lockedHours.has(h) // 👈 add the flag so your embed renderer can show 🔒
  }));
...
export async function onButton(interaction) {
  ...
  // buff manager (single-hour action → open user picker)
  ...
  // buff manager (multi-hour action → open user picker)
  ...
  // user self-service entry buttons
  if (
  ...
  // guard: king_grant / king_revoke are handled in onSelectMenu
  if (interaction.customId === 'king_grant' || interaction.customId === 'king_revoke') return;
}

/* ========================== SELECT MENU HANDLER ========================== */

export async function onSelectMenu(interaction) {
  // route King user-selects here
  if (interaction.customId === 'king_grant' || interaction.customId === 'king_revoke') {
    await onUserSelectKing(interaction);
    return;
  }

  // date selected on public panel
  ...
  if (['add_hours_submit', 'remove_hours_submit', 'edit_hours_submit'].some(p => interaction.customId.startsWith(p))) {
    ...
    if ((action === 'add_hours_submit' || action === 'edit_hours_submit')) {
      if (hours.length < 2) {
        await interaction.reply({ content: '⚠️ You must select **at least 2 hours**.', flags: MessageFlags.Ephemeral });
        return;
      }
      let consecutive = true;
      for (let i = 1; i < hours.length; i++) {
        if (hours[i] - hours[i - 1] !== 1) { consecutive = false; break; }
      }
      if (!consecutive) {
        await interaction.reply({ content: '⚠️ Please select **consecutive hours** (e.g., 13:00–15:00).', flags: MessageFlags.Ephemeral });
        return;
      }
    }

    // block on King-unavailable
  ...
  // Buff Manager: pick date → multi-hours
  if (interaction.customId === 'bm_date') {
    ...
    await interaction.reply({ content: `📅 Date selected: **${date}**. Now pick **one or more hours**:`, components: [row], flags: MessageFlags.Ephemeral });
    return;
  }

  // Buff Manager: pick hour → date
  if (interaction.customId === 'bm_hour') {
    ...
    await interaction.reply({ content: `🕑 Hour selected: **${String(hour).padStart(2,'0')}:00**. Now pick a **date**:`, components: [row], flags: MessageFlags.Ephemeral });
    return;
  }

  // Buff Manager: date→single-hour actions
  if (interaction.customId.startsWith('bm2_hour:')) {
    ...
    await interaction.reply({ content: `📌 Managing **${date} ${String(hour).padStart(2,'0')}:00 UTC**. Choose an action:`, components: [row], flags: MessageFlags.Ephemeral });
    return;
  }

  // Buff Manager: hour→date actions
  ...
  // Buff Manager: date→multi-hours actions
  if (interaction.customId.startsWith('bm2_hours:')) {
    ...
    await interaction.reply({ content: `📌 Managing **${date}** hours **${hours.map(h=>String(h).padStart(2,'0')).join(', ')}:00 UTC**. Choose an action:`, components: [row], flags: MessageFlags.Ephemeral });
    return;
  }

  // King Unavailable: pick date
  if (interaction.customId === 'kb_date') {
    ...
    await interaction.reply({ content: `📅 Select the hours to **mark unavailable** for **${date} (UTC)**:`, components: [row], flags: MessageFlags.Ephemeral });
    return;
  }

  // King Unavailable: choose hours → buttons
  if (interaction.customId.startsWith('kb_hours:')) {
    ...
    await interaction.reply({ content: `📌 **${date}** — hours **${hours.map(h=>String(h).padStart(2,'0')).join(', ')}:00 UTC**. Choose an action:`, components: [row], flags: MessageFlags.Ephemeral });
    return;
  }
  ...
  if (interaction.customId.startsWith('bm_pick:')) {
    ...
    try {
      ...
      await interaction.reply({ content: `✅ Updated **${date} ${String(hour).padStart(2,'0')}:00 UTC** (${action}).`, flags: MessageFlags.Ephemeral });
    } catch (e) {
      await interaction.reply({ content: `⚠️ Failed to update slot: ${e.message || e}`, flags: MessageFlags.Ephemeral });
    }
    return;
  }
  ...
  if (interaction.customId.startsWith('bm_pick_multi:')) {
    ...
    try {
      ...
      await refreshDayEmbed(interaction.client, guildId, date);
      await interaction.reply({ content: `✅ Updated **${date}** hours **${hours.map(h=>String(h).padStart(2,'0')).join(', ')}:00 UTC** (${action}).`, flags: MessageFlags.Ephemeral });
    } catch (e) {
      await interaction.reply({ content: `⚠️ Failed to update selected hours: ${e.message || e}`, flags: MessageFlags.Ephemeral });
    }
    return;
  }
}

/* ========================== KING USER-SELECT HANDLER ========================== */
...
export async function onUserSelectKing(interaction) {
  try {
    ...
    if (grant) {
      // Remove King from everyone not selected (also remove Buff)
      ...
      // Grant King to selected and give Buff role too
    ...
    // Revoke branch: remove King (and Buff) from selected
    for (const uid of selectedIds) {
      try {
        const m = await guild.members.fetch(uid);
        if (m.roles.cache.has(kingRole.id)) {
