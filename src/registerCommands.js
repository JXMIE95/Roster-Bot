...
const cmds = [
  ...
  new SlashCommandBuilder().setName('config').setDescription('Configure bot settings')
    ...
    .addSubcommand(s => s.setName('kinglead').setDescription('Set King change-lead minutes')
      .addIntegerOption(o => o.setName('minutes').setDescription('Minutes').setRequired(true)))
    .addSubcommand(s => s.setName('buffrole').setDescription('Set the Buff Giver role (active slot role)')
      .addRoleOption(o => o.setName('role').setDescription('Role').setRequired(true))),
  ...
  new SlashCommandBuilder().setName('kingnotify').setDescription('King utilities')
    ...
    .addSubcommand(s => s.setName('slot').setDescription('Notify a specific slot (DMs)')
      .addStringOption(o => o.setName('date').setDescription('YYYY-MM-DD').setRequired(true))
...
async function main() {
  ...
  const guildId = process.env.GUILD_ID; // if present, register to guild for instant availability
